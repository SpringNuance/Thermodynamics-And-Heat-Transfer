...
for iii=1:size(indx,2)
    SD.D(indx(iii)).EffDeweCHR = SD.D(indx(iii)).CHR(:,1700:2000)';
    SD.D(indx(iii)).AvgDeweCHR = mean(SD.D(indx(iii)).EffDeweCHR,2);

    SD.D(indx(iii)).OReCHR = SD.D(indx(iii)).ReCHR(1700:2000,:);
...
for iii=1:size(indx,2)
    ...
    SD.D(indx(iii)).EffHRR = SD.D(indx(iii)).dQSmo(1700:2000,:); % with own method
    SD.D(indx(iii)).AvgHRR = mean(SD.D(indx(iii)).EffHRR,2);

    SD.D(indx(iii)).DeweHRR = SD.D(indx(iii)).HRR(:,1700:2000)'; % directly from Dewesoft
