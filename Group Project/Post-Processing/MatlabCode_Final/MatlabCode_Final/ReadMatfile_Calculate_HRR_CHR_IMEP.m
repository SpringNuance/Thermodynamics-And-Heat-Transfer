...
for kk = 1 : numberOfFolders
    ...
    if numberOfMatFiles >= 1   % Go through all those files.
        ...
        for f = 1 : numberOfMatFiles
            ...
            SD(kk).D(f).HRR= D(kk).D(f).Data.Data1_dQ1;% Heat Release Rate from Dewesoft
            SD(kk).D(f).CHR=D(kk).D(f).Data.Data1_T2_IntQ1;% Cummulative heat release from Dewesoft
            SD(kk).D(f).InjCur=D(kk).D(f).Data.Data1_Injection_signal_1; % Injection current from Dewesoft

            SD(kk).S(f).Torque= D(kk).D(f).Data.Data1_Torque1; % Torque of each cycle f rom Dewesoft
            SD(kk).S(f).Power= D(kk).D(f).Data.Data1_Power1;% Power of each cyclefrom Dewesoft
            SD(kk).S(f).CA5= D(kk).D(f).Data.Data1_MFB05_1; % 5% fuel burning rate from Dewesoft
            SD(kk).S(f).CA10= D(kk).D(f).Data.Data1_MFB10_1; % 10% fuel burning rate from Dewesoft
            SD(kk).S(f).CA50= D(kk).D(f).Data.Data1_MFB50_1; % 50% fuel burning rate from Dewesoft
            SD(kk).S(f).CA90= D(kk).D(f).Data.Data1_MFB90_1; % 90% fuel burning rate from Dewesoft
            SD(kk).S(f).SOC= D(kk).D(f).Data.Data1_SOC1; % Start of combustion from Dewesoft
...
for t= 1:size(SD,2)
    for s=1:size(SD.D,2)
        ...
        for u=1700:2100
            ...
            SD(t).D(s).ReCHR(u,:)=SD(t).D(s).ReCHR(u-1,:)+SD(t).D(s).dQSmo(u,:)*dtheta; %cumulQ is cumulative heat release
        end
